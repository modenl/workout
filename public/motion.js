...
const Motion = (() => {
  ...
  const sideViews=new Set(["stand","miniSquat","extend","wallPush","forwardPress","reachTap","elbowPull","lowRow","backLeg","toeLift","seatedHeel"]);
  const standing=new Set(["stand","miniSquat","wallPush","heel","side","weightShift","backLeg"]);
  ...
  function pose(id,t,side=1) {
    ...
    for(const s of [-1,1]){
      ...
      const legActive=["stand","miniSquat","heel","toeLift","seatedHeel","seatedJack","weightShift"].includes(id)||(["march","extend","reachTap","crossMarch","kneePress","side","backLeg"].includes(id)&&active);
      ...
      if(["miniSquat","heel","side","weightShift","backLeg"].includes(id)){hand=[s*30,190,80];}
    ...
    for(const a of arms){[a.elbow,a.hand]=ik(a.sh,a.hand,56,54,a.pole);}
  ...
  function render(ctx,p,cx,cy,scale,small){
    ...
    if(["miniSquat","heel","side","weightShift","backLeg"].includes(p.id))chair(115,86,190);
