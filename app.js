...
function standing(ctx, c, opt = {}) {
  ...
  const footL={x:c.x-34+(opt.sideL||0),y:c.y}, footR={x:c.x+34+(opt.sideR||0),y:c.y};
...
const drawers = {
  ...
  push(ctx,c,t){
    ...
    const shift=48*t, hip={x:c.x-45+shift,y:c.y-135},shoulder={x:c.x-75+shift,y:c.y-255},hd={x:shoulder.x-10,y:shoulder.y-49};
  ...
  heel(ctx,c,t){
    chair(ctx,c.x+85,c.y,1); const rise=-28*t; const fig=standing(ctx,{x:c.x,y:c.y},{rise});
